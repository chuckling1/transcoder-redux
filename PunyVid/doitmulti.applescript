...
on clicked theObject
	...
	repeat with theRow in every data row of data column "Files" of theList
		...
		if contents of text field "path" of window "PunyVid" = "" then
			...
			set mypath to theFilepath
		else
			set destpath to (quoted form of (contents of text field "path" of window "PunyVid" as string))
			set mypath to contents of text field "path" of window "PunyVid" as string
		end if
		--Assemble some nice looking text strings.
		set outputfile to quoted form of POSIX path of (mypath & theFile & ".temp.mp4")
		...
		if ext = "m4v" then
			...
			try
				if decimal is not "comma" then
					set tempfps to (do shell script thequotedapppath & "/Contents/Resources/movinfo " & thequotedorigpath & " | /usr/bin/awk '{print $4}'")
					set tempfpslen to (count of characters in tempfps) as number
					set tempfpslen to tempfpslen - 1
					set tempfps to characters 1 through tempfpslen of tempfps as string
					set movfps to (round tempfps)
				else
					set movfps2 to (do shell script thequotedapppath & "/Contents/Resources/movinfo " & thequotedorigpath & " | /usr/bin/awk '{print $4}' | sed -e 's/\\./,/g'") as number
					set movfps to (round movfps2)
				end if
